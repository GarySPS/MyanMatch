...
export default function VoicePromptPage() {
  ...
  const [audioURL, setAudioURL] = useState(null);   // DB url or fresh blob URL
  const [audioBlob, setAudioBlob] = useState(null); // when recorded now
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const uid = local?.user_id || local?.id;
        ...
        if (!error && data?.voicePrompt) {
          ...
          if (vp?.prompt) {
            // Map existing literal back to a key if possible
            const maybeKey = VOICE_PROMPT_KEYS.find(k => t(`vp.pool.${k}`) === vp.prompt);
    ...
    >
      ...
      <div className="relative bg-white/95 rounded-3xl shadow-2xl max-w-md w-full px-7 py-10 flex flex-col items-center">
        ...
        <div className="rounded-2xl border-2 border-dashed border-[#ecd8f5] px-4 py-7 flex flex-col items-center justify-center bg-white/80 relative min-h-[120px] mb-3 w-full">
          ...
          {loading ? (
            <div className="text-sm text-gray-500">{t("vp.loading")}</div>
          ) : audioURL ? (
            <>
        ...
        <button
          ...
          onClick={() => {
            ...
            setProfileData((prev) => ({
              ...prev,
              voicePrompt: file
                ? { prompt: selectedPromptLabel, audioBlob, file, audioURL, duration }
                : { prompt: selectedPromptLabel, url: audioURL, duration },
            }));
            navigate("/onboarding/finish");
          }}
          disabled={!audioURL}
