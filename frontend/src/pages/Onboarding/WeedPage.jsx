...
  const { t } = useI18n();
  // This state will now hold the KEY, e.g., "sometimes" instead of the translated text
...
    setProfileData((prev) => ({
      ...prev,
      // [!FIX!] The canonical key is now being saved to the context.
      weed: value,
    }));
...
            const label = t(`weed.opt.${k}`);
            // [!FIX!] The comparison is now against the key `k`.
...
                  }
                `}
                // [!FIX!] The value being set is the key `k`, not the `label`.
                onClick={() => setValue(k)}
              >
