// src/pages/Onboarding/PoliticalBeliefPage.jsx
...
const OPTIONS = [
  { key: "pol.opt.liberal",    value: "Liberal" },
  { key: "pol.opt.moderate",    value: "Moderate" },
  { key: "pol.opt.conservative",value: "Conservative" },
  { key: "pol.opt.notPolitical",value: "Not political" },
  { key: "pol.opt.other",       value: "Other" },
  { key: "pol.opt.na",          value: "Prefer not to say" },
];

export default function PoliticalBeliefPage() {
  const navigate = useNavigate();
  const { setProfileData } = useOnboarding();
  const { t } = useI18n();
  
  // This will now store the canonical value, e.g., "Liberal"
...
    setProfileData((prev) => ({
      ...prev,
      political_belief: value, // This is now the consistent, non-translated value
    }));
    navigate("/onboarding/drinking");
  };
...
            <HiOutlineOfficeBuilding className="w-5 h-5 text-[#6e2263]" />
          </div>
...
                    selected
                      ? "border-[#a55596] bg-[#a55596]"
                      : "border-gray-300 bg-white"
                  }`}
                >
                  {selected && (<svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><circle cx="12" cy="12" r="8" fill="currentColor" /></svg>)}
                </span>
              </button>
            );
          })}
